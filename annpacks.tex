...
\end{minted}
\end{enumerate}
\subsubsection{Implementing typelists using annotated packs}
\p Accessors can be used with parameter packs in order to access individual elements in a non-annotated template parameter pack as the ones currently present in C++.
The following examples implement individual access and change for types contained within parameter packs in a class template used as type vector.
...
We will do so by first definining a set of class templates whose partial specializations will provide the pattern matching constructs which we will exploit through substitutive ordering in order to convert non-annotated parameter pack operations into annotated versions.
\begin{minted}{c++}

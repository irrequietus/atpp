...
As a consequence, several errors, omissions, repetitions or inadequate development of the notions involved are to be taken care of in future releases of this document.
...
\end{minted}
\end{enumerate}
\subsubsection{Combining with different kinds of expansions}
...
\subsubsection{The C++11/C++14 block switch effect, better}
\p Another example can be made with function templates and lambdas, implementing a peculiar kind of "switch" as in the following, valid C++11/C++14 code snippet where lambdas and sfinae are combined to provide a "block switch" effect within the body of the \textit{func\_templ} function template depending on the number of parameters present in a pack.
...
template<typename... T>
std::enable_if_t<(sizeof...(T) >= 1) && (sizeof...(T) < 6),void>
function(T... t) {
...
\p Given than an invalid anchored pack expansion when the pack identifier has been declared with interval annotation removes the template where it is present from the resolution set, it is fairly easy to avoid even the use of \textit{std::enable\_if} C++11/C++14 template when defining the \textit{func\_tmpl} function template and the \textit{sel} helpers.

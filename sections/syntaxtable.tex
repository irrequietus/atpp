...
If and only if it is not followed by the \textit{tuple} annotator and $sizeof...(T)$ is implied, it may be omitted since it represents a form equivalent to a non-annotated template parameter pack.
...
When no expression is specified, $\bm{1}$ is implied and it may be omitted since it represents a form equivalent to an \textit{size}-annotated pack.
In declaration, the \textit{tuple} annotator requires the pack it is applied upon to be an exact multiple of an ordered sequence of types, the latter of size equal to the integral constant expression it encloses. In expansion, it repeats the annotated pack as many times as said expression.
The \textit{tuple} annotator is a valid construct if and only if preceded by a valid \textit{size} annotator.
